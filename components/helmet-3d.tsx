...
function Helmet3DScene() {
  ...
  useEffect(() => {
    ...
    const helmetTexture = textureLoader.load("/images/helmet.png")
    ...
    const spotLight1 = new THREE.SpotLight(0xc0c0c0, 2)
    ...
    const animate = () => {
      ...
      // Rotate the helmet in 3D space (same as bootie)
      helmet.rotation.y = Math.sin(time * 0.5) * 0.5
      helmet.rotation.x = Math.cos(time * 0.3) * 0.3
      helmet.rotation.z = Math.sin(time * 0.2) * 0.1

      // Float animation
      helmet.position.y = Math.sin(time * 0.8) * 0.3
